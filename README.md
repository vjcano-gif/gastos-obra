...
guarda la evidencia y muestra gastos vs ingresos por proyecto. Un solo usuario.

## Arquitectura

- **app/** — Streamlit (Streamlit Cloud): Revisión, Dashboard, Cuentas por pagar,
  Estado de cuenta, Configuración.
- **worker/** — barrido del buzón cada 6 horas (GitHub Actions).
- **supabase/** — migraciones SQL (Postgres + Storage privado, RLS por usuario).

## Montaje (una sola vez por cliente)

1. **Supabase**: crear proyecto nuevo → SQL Editor → pegar
   `supabase/migrations/001_esquema.sql` → Run.
   En Authentication → Users → crear el usuario operador (correo + contraseña).
